...
export function useSeasonalFieldWindow(): FieldWindow {
  ...
  return useMemo(() => {
    ...
    if (memoryDepth === 'deep' && soul.isEstablished && soul.soulLine) {
      const line = soul.soulLine;
      suggestion = line.charAt(0).toUpperCase() + line.slice(1) + '. ' + base;
    }

    if (mode === 'love' && quality === 'avoid') {
      suggestion = 'Rest period. The field is resting too. Good time to reflect on what you have seen.';
    }
